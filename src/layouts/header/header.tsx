...
export default function Header({ className }: { className?: string }) {
  ...
  return (
    <nav
      className={cn(
        'sticky top-0 z-30 h-16 w-full bg-dark transition-all duration-300 ltr:right-0 rtl:left-0 sm:h-20 3xl:h-24',
        (isMounted && windowScroll.y) > 2
          ? 'bg-dark bg-gradient-to-b from-dark to-dark/80 backdrop-blur'
          : '',
        className
      )}
    >
      <div className="flex h-full items-center justify-between px-4 sm:px-6 lg:px-8 3xl:px-10">
        <div className="flex items-center">
          <div
            onClick={() => router.push(routes.portafolio)}
